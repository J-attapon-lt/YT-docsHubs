...
const getImageUrl = (url = '') => {
  ...
  if (fileMatch?.[1]) return `https://drive.google.com/thumbnail?id=${fileMatch[1]}&sz=w1600`;
...
const getSectionImages = (section) => {
  ...
  const legacyImage = section?.imageUrl ? [{ url: section.imageUrl, caption: section.imageCaption || 'ภาพประกอบ' }] : [];
  return [...images, ...legacyImage].filter((image) => image?.url);
};
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col relative">
      ...
      <div className="flex-1 max-w-5xl mx-auto w-full px-6 py-12">
        <h2 className="text-2xl font-bold text-gray-800 mb-8 flex items-center gap-2">
          <BookOpen className="w-6 h-6 text-blue-600" /> คลังเอกสาร ({filteredDocs.length})
        </h2>
        {filteredDocs.length > 0 ? (
            ...
            {filteredDocs.map((item) => (
              <button key={item.id} onClick={() => onSelectDoc(item)} className="group flex flex-col text-left bg-white rounded-2xl p-6 shadow-sm hover:shadow-xl border border-gray-100 transition-all duration-300 transform hover:-translate-y-1 hover:border-blue-200">
                ...
                <div className="mt-6 flex items-center justify-between text-sm font-semibold text-blue-600 bg-blue-50 px-4 py-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity">
                  <span>เปิดอ่าน</span><ChevronRight className="w-4 h-4" />
                </div>
              </button>
            ))}
          </div>
          ...
          <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-gray-300">
            ...
            <h3 className="text-lg font-bold text-gray-600">ไม่พบคู่มือที่คุณค้นหา</h3>
          </div>
        )}
      </div>
...
const ReaderView = ({ docData, onBack }) => {
  ...
  const printAllSections = () => {
    ...
    const allContent = (docData.sections || []).map((section) => {
      const imagesHtml = getSectionImages(section).map((image, imageIndex) => `
        <figure style="margin-top:24px;text-align:center;page-break-inside:avoid;">
          ...
          <figcaption style="font-size:13px;color:#666;margin-top:8px;">${image.caption || `ภาพที่ ${imageIndex + 1}`}</figcaption>
        </figure>
      `).join('');
  ...
  if (!docData.sections?.length) {
    return <div className="h-screen flex flex-col items-center justify-center bg-gray-50"><h2 className="text-2xl font-bold text-gray-800 mb-4">ยังไม่มีเนื้อหาในคู่มือนี้</h2><button onClick={onBack} className="bg-blue-600 text-white px-6 py-2 rounded-lg">กลับหน้าหลัก</button></div>;
  }

  const currentImages = getSectionImages(currentContent);
  ...
  return (
    <div className="flex h-screen bg-gray-50 font-sans">
      ...
      <div className={`fixed inset-y-0 left-0 z-30 w-80 bg-white border-r shadow-lg transform transition-transform duration-300 lg:relative lg:translate-x-0 flex flex-col no-print ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className={`p-5 border-b flex justify-between items-center ${docData.color || 'bg-blue-600'} text-white`}>
          <div><h1 className="text-lg font-bold tracking-tight line-clamp-2 leading-tight">{docData.title}</h1><p className="text-[10px] opacity-80 mt-1 uppercase tracking-wider font-semibold">Table of Contents</p></div>
          <button className="lg:hidden" onClick={() => setIsSidebarOpen(false)}><X className="w-6 h-6 text-white" /></button>
        </div>
        <div className="flex-1 overflow-y-auto py-4">
          <div className="px-3 mb-3"><div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-4 h-4" /><input value={sectionSearch} onChange={(event) => setSectionSearch(event.target.value)} placeholder="ค้นหาในคู่มือ..." className="w-full pl-9 pr-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" /></div></div>
          <ul className="space-y-1 px-3">
            {filteredSections.map((item) => <li key={item.id}><button onClick={() => handleSelectSection(item.id)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-all ${activeSection === item.id ? 'bg-blue-50 text-blue-700 font-bold border-l-4 border-blue-600 shadow-sm' : 'text-gray-600 hover:bg-gray-100 border-l-4 border-transparent'}`}><span className={activeSection === item.id ? 'text-blue-600' : 'text-gray-400'}><BookOpen className="w-4 h-4" /></span><span className="text-sm leading-5">{item.title}</span></button></li>)}
          </ul>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-white print-area">
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-12 print-area"><div className="max-w-4xl mx-auto">
          <div className="mb-8"><h1 className="text-3xl md:text-4xl font-extrabold text-gray-900 pb-4 border-b-2 border-blue-100 mt-4 leading-tight">{currentContent?.title}</h1></div>
          <div className="prose prose-blue max-w-none text-base lg:text-lg text-gray-800 print-content space-y-4" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(currentContent?.content || '') }} />
          {currentImages.length > 0 && (
            <div className="mt-8 space-y-8">
              {currentImages.map((image, imageIndex) => (
                <figure key={`${image.url}-${imageIndex}`} className="rounded-2xl border bg-gray-50 p-4 shadow-sm">
                  <img src={getImageUrl(image.url)} alt={image.caption || `ภาพที่ ${imageIndex + 1}`} className="mx-auto max-h-[520px] w-full object-contain rounded-xl border bg-white" onError={(event) => { event.currentTarget.style.display = 'none'; }} />
                  <figcaption className="mt-3 text-center text-sm text-gray-500">{image.caption || `ภาพที่ ${imageIndex + 1}`}</figcaption>
                </figure>
              ))}
            </div>
          )}
          <div className="mt-20 pt-8 border-t flex justify-between items-center no-print bg-white">
            {(() => { const index = docData.sections.findIndex((item) => item.id === activeSection); const prev = index > 0 ? docData.sections[index - 1] : null; const next = index < docData.sections.length - 1 ? docData.sections[index + 1] : null; return <><div className="flex-1">{prev && <button onClick={() => handleSelectSection(prev.id)} className="text-left text-gray-500 hover:text-blue-700 p-4"><span className="text-xs font-bold uppercase mb-1 block">« บทก่อนหน้า</span><span className="font-medium text-lg">{prev.title}</span></button>}</div><div className="flex-1 text-right">{next ? <button onClick={() => handleSelectSection(next.id)} className="text-right text-gray-500 hover:text-blue-700 p-4 ml-auto"><span className="text-xs font-bold uppercase mb-1 block">บทถัดไป »</span><span className="font-medium text-lg">{next.title}</span></button> : <button onClick={onBack} className="text-right text-gray-500 hover:text-green-700 p-4 ml-auto flex flex-col items-end"><span className="text-xs font-bold uppercase mb-1">จบเนื้อหา</span><span className="font-medium text-lg flex items-center gap-2 text-green-700">กลับสู่หน้าหลัก<CheckCircle2 className="w-5 h-5" /></span></button>}</div></>; })()}
          </div>
        </div></div>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6 relative">
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden z-10">
        <div className="bg-blue-900 text-white p-8 text-center border-b border-blue-800"><div className="flex justify-center mb-6"><CompanyLogo textClassName="text-white" /></div><h2 className="text-xl font-bold">เข้าสู่ระบบหลังบ้าน</h2><p className="text-blue-200 text-sm mt-1">Admin Dashboard Login</p></div>
        <form onSubmit={handleLogin} className="p-8 space-y-6">
          {error && <div className="bg-red-50 text-red-600 p-3 rounded-lg flex items-center gap-2 text-sm"><AlertCircle className="w-5 h-5 flex-shrink-0" /><span>{error}</span></div>}
          <div className="space-y-4">
            <div><label className="block text-sm font-medium text-gray-700 mb-1">อีเมลแอดมิน</label><div className="relative"><Mail className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5" /><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} required className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all" placeholder="admin@ltrthailand.com" /></div></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">รหัสผ่าน</label><div className="relative"><Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5" /><input type="password" value={password} onChange={(event) => setPassword(event.target.value)} required className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all" placeholder="••••••••" /></div></div>
          </div>
...
const AdminEditor = ({ initialDoc, onCancel, onSave }) => {
  ...
  const addSection = () => {
    setForm((prev) => ({
      ...prev,
      sections: [...(prev.sections || []), { id: `section-${Date.now()}`, title: `บทใหม่ ${(prev.sections || []).length + 1}`, content: '<p>ใส่เนื้อหาที่นี่...</p>', images: [] }],
    }));
  };
  ...
  const addImage = (sectionIndex) => {
    setForm((prev) => {
      ...
      sections[sectionIndex] = { ...sections[sectionIndex], images: [...currentImages, { url: '', caption: `ภาพที่ ${currentImages.length + 1}` }] };
      return { ...prev, sections };
    });
  };
  ...
  const handleSubmit = async (event) => {
    event.preventDefault();
    if (!form.id.trim()) return alert('กรุณาระบุ Document ID');
    if (!form.title.trim()) return alert('กรุณาระบุชื่อคู่มือ');
    setSaving(true);
    try { await onSave({ ...form, id: form.id.trim(), updatedAt: new Date().toISOString() }); }
    finally { setSaving(false); }
  };

  return (
    <div className="min-h-screen bg-gray-50 font-sans">
      <div className="bg-blue-900 text-white px-6 py-4 flex items-center justify-between shadow"><CompanyLogo textClassName="text-white" /><button onClick={onCancel} className="bg-white/10 hover:bg-white/20 px-4 py-2 rounded-lg text-sm flex items-center gap-2"><ArrowLeft className="w-4 h-4" />กลับ Dashboard</button></div>
      <form onSubmit={handleSubmit} className="max-w-6xl mx-auto px-6 py-10 space-y-8">
        <div><h1 className="text-3xl font-extrabold text-gray-900">{initialDoc ? 'แก้ไขคู่มือ' : 'เพิ่มคู่มือใหม่'}</h1><p className="text-gray-500 mt-1">กรอกข้อมูลคู่มือและจัดการบทภายในเอกสาร</p></div>
        <div className="bg-white rounded-2xl shadow-sm border p-6 space-y-6">
          <h2 className="text-xl font-bold text-gray-900 border-b pb-3">ข้อมูลคู่มือ</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Input label="Document ID" value={form.id} onChange={(value) => updateField('id', value)} disabled={!!initialDoc} placeholder="akrivis-warehouse" hint="ใช้ตัวอักษรอังกฤษ ตัวเลข และขีดกลาง" />
            <Select label="สีการ์ด" value={form.color} onChange={(value) => updateField('color', value)} options={[['bg-blue-700', 'น้ำเงิน'], ['bg-green-700', 'เขียว'], ['bg-purple-700', 'ม่วง'], ['bg-orange-600', 'ส้ม'], ['bg-red-700', 'แดง'], ['bg-gray-800', 'เทาเข้ม']]} />
            <div className="md:col-span-2"><Input label="ชื่อคู่มือ" value={form.title} onChange={(value) => updateField('title', value)} placeholder="ศูนย์รวมคู่มือการใช้งานและเอกสารอ้างอิงของระบบ" /></div>
            <div className="md:col-span-2"><label className="block text-sm font-bold text-gray-700 mb-2">คำอธิบาย</label><textarea value={form.description} onChange={(event) => updateField('description', event.target.value)} rows={3} className="w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" /></div>
            <Select label="ไอคอน" value={form.iconName} onChange={(value) => updateField('iconName', value)} options={[['BookOpen', 'BookOpen'], ['Warehouse', 'Warehouse'], ['MonitorSmartphone', 'MonitorSmartphone'], ['Settings', 'Settings'], ['FileText', 'FileText']]} />
            <Select label="สถานะเอกสาร" value={form.status || 'draft'} onChange={(value) => updateField('status', value)} options={[['draft', 'Draft - ฉบับร่าง'], ['published', 'Published - เผยแพร่']]} />
          </div>
        </div>

        <div className="bg-white rounded-2xl shadow-sm border p-6 space-y-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b pb-4"><div><h2 className="text-xl font-bold text-gray-900">เนื้อหาบททั้งหมด</h2><p className="text-sm text-gray-500 mt-1">สามารถใส่ HTML ได้ เช่น &lt;p&gt;, &lt;h3&gt;, &lt;ul&gt;, &lt;table&gt;</p></div><button type="button" onClick={addSection} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl font-bold flex items-center gap-2"><Plus className="w-4 h-4" />เพิ่มบท</button></div>
          {form.sections.length === 0 ? <div className="border border-dashed rounded-2xl p-10 text-center text-gray-500"><BookOpen className="w-12 h-12 text-gray-300 mx-auto mb-3" /><p>ยังไม่มีบทในคู่มือนี้</p></div> : (
            <div className="space-y-6">
              {form.sections.map((section, index) => (
                <div key={`${section.id}-${index}`} className="border rounded-2xl overflow-hidden">
                  <div className="bg-gray-50 px-5 py-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3 border-b"><div className="font-bold text-gray-800">บทที่ {index + 1}</div><div className="flex items-center gap-2"><button type="button" onClick={() => moveSection(index, -1)} className="px-3 py-1 bg-white border rounded-lg text-sm hover:bg-gray-100">ขึ้น</button><button type="button" onClick={() => moveSection(index, 1)} className="px-3 py-1 bg-white border rounded-lg text-sm hover:bg-gray-100">ลง</button><button type="button" onClick={() => deleteSection(index)} className="px-3 py-1 bg-red-50 text-red-600 rounded-lg text-sm hover:bg-red-100">ลบ</button></div></div>
                  <div className="p-5 space-y-4">
                    <Input label="Section ID" value={section.id} onChange={(value) => updateSection(index, 'id', value)} />
                    <Input label="ชื่อบท" value={section.title} onChange={(value) => updateSection(index, 'title', value)} />
                    <div><label className="block text-sm font-bold text-gray-700 mb-2">เนื้อหา HTML</label><textarea value={section.content} onChange={(event) => updateSection(index, 'content', event.target.value)} rows={10} className="w-full px-4 py-3 border rounded-xl font-mono text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" /></div>

                    <div className="border rounded-xl p-5 bg-blue-50/40">
                      <div className="flex items-center justify-between gap-3 mb-4"><div><h3 className="text-sm font-bold text-gray-700">ภาพประกอบหลายภาพ</h3><p className="text-xs text-gray-500 mt-1">วางลิงก์ Google Drive หรือ URL รูปภาพได้หลายรายการ</p></div><button type="button" onClick={() => addImage(index)} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-bold">+ เพิ่มภาพ</button></div>
                      {(section.images || []).length === 0 ? <div className="border border-dashed rounded-xl p-6 text-center text-gray-400 bg-white">ยังไม่มีภาพประกอบ</div> : (
                        <div className="space-y-4">
                          {(section.images || []).map((image, imageIndex) => (
                            <div key={imageIndex} className="bg-white border rounded-xl p-4 space-y-3">
                              <div className="flex items-center justify-between"><p className="font-bold text-gray-700">ภาพที่ {imageIndex + 1}</p><button type="button" onClick={() => deleteImage(index, imageIndex)} className="px-3 py-1 bg-red-50 text-red-600 rounded-lg text-sm hover:bg-red-100">ลบภาพ</button></div>
                              <div><label className="block text-xs font-bold text-gray-600 mb-1">URL ภาพจาก Google Drive</label><input type="url" value={image.url || ''} onChange={(event) => updateImage(index, imageIndex, 'url', event.target.value)} placeholder="https://drive.google.com/file/d/FILE_ID/view?usp=sharing" className="w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" /></div>
                              <div><label className="block text-xs font-bold text-gray-600 mb-1">คำอธิบายใต้ภาพ</label><input type="text" value={image.caption || ''} onChange={(event) => updateImage(index, imageIndex, 'caption', event.target.value)} placeholder={`ภาพที่ ${imageIndex + 1} หน้าจอ...`} className="w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" /></div>
                              {image.url && <figure className="rounded-xl border bg-gray-50 p-3"><img src={getImageUrl(image.url)} alt={image.caption || `ภาพที่ ${imageIndex + 1}`} className="max-h-72 w-full object-contain rounded-lg border bg-white" onError={(event) => { event.currentTarget.style.display = 'none'; }} />{image.caption && <figcaption className="text-center text-sm text-gray-500 mt-2">{image.caption}</figcaption>}</figure>}
                            </div>
                          ))}
                        </div>
                      )}
                    ...
                    <div className="bg-gray-50 border rounded-xl p-5"><p className="text-xs font-bold text-gray-500 uppercase mb-3 flex items-center gap-2"><Eye className="w-4 h-4" />Preview</p><div className="prose prose-blue max-w-none text-gray-800" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(section.content || '') }} />{getSectionImages(section).length > 0 && <div className="mt-4 space-y-4">{getSectionImages(section).map((image, imageIndex) => <figure key={`${image.url}-${imageIndex}`} className="rounded-xl border bg-white p-3"><img src={getImageUrl(image.url)} alt={image.caption || `ภาพที่ ${imageIndex + 1}`} className="max-h-72 w-full object-contain rounded-lg border bg-white" />{image.caption && <figcaption className="text-center text-sm text-gray-500 mt-2">{image.caption}</figcaption>}</figure>)}</div>}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="sticky bottom-0 bg-white border shadow-lg rounded-2xl p-4 flex justify-end gap-3"><button type="button" onClick={onCancel} className="px-6 py-3 border rounded-xl font-bold text-gray-600 hover:bg-gray-50">ยกเลิก</button><button type="submit" disabled={saving} className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold flex items-center gap-2 disabled:opacity-60"><Save className="w-5 h-5" />{saving ? 'กำลังบันทึก...' : 'บันทึกคู่มือ'}</button></div>
      </form>
    </div>
  );
};

const Input = ({ label, value, onChange, placeholder = '', disabled = false, hint = '' }) => (
  <div><label className="block text-sm font-bold text-gray-700 mb-2">{label}</label><input type="text" value={value || ''} onChange={(event) => onChange(event.target.value)} disabled={disabled} placeholder={placeholder} className="w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none disabled:bg-gray-100" />{hint && <p className="text-xs text-gray-400 mt-1">{hint}</p>}</div>
);

const Select = ({ label, value, onChange, options }) => (
  <div><label className="block text-sm font-bold text-gray-700 mb-2">{label}</label><select value={value} onChange={(event) => onChange(event.target.value)} className="w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none">{options.map(([v, t]) => <option key={v} value={v}>{t}</option>)}</select></div>
);
...
export default function App() {
  ...
  useEffect(() => {
    ...
    return onAuthStateChanged(auth, async (currentUser) => {
      ...
      else { setUserRole(null); setUserProfile(null); }
    });
  }, []);
